...
function createOrganizationsEndpoints ({ apiKey = '', target = 'api', basePath = '/' }) {
  ...
  function listOrganizations () {
    const path = `${basePath.split('organizations/')[0]}/organizations`
    return axios._get(apiKey, target, path)
  }
   ...
   * }
   */
  function updateOrganization ({ name }) {
    const data = { name }
    return axios._put(apiKey, target, basePath, data)
  }
   ...
   * @memberof module:lib/rest/organizations
   * @param { Object } param                  The parameters for this request
   * @param { string } param.organizationId   The id of the organization to clone
   ...
   * }
   */
  function cloneOrganization ({ organizationId, name }) {
    if (!organizationId) {
      return Promise.reject(new Error('The parameter organizationId is mandatory'))
    }

    const data = { name }
    return axios._post(apiKey, target, `${basePath}/clone`, data)
  }
   ...
   */
  function claimOrganization ({ order, serial, licenseKey, licenseMode }) {
    const data = { order, serial, licenseKey, licenseMode }
    return axios._post(apiKey, target, `${basePath}/claim`, data)
  }
  ...
  function showOrganizationLicense () {
    return axios._get(apiKey, target, `${basePath}/licenseState`)
  }
  ...
  function listOrganizationInventory () {
    return axios._get(apiKey, target, `${basePath}/inventory`)
  }
  ...
  function showOrganizationSNMP () {
    return axios._get(apiKey, target, `${basePath}/snmp`)
  }
  ...
  function updateOrganizationSNMP (data) {
    return axios._post(apiKey, target, `${basePath}/snmp`, data)
  }
