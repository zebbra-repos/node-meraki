 ...
 * const adminEndpoints = require('./lib/rest/networks')({ apiKey, target, basePath, organizationId })
 */
function createNetworksEndpoints ({ apiKey = '', target = 'api', basePath = '/', organizationId = '' }) {
  /**
   ...
   * ]
   */
  function listNetworks ({ configTemplateId }) {
    const params = { configTemplateId }
    return axios._get(apiKey, target, `${basePath}/organizations/${organizationId}/networks`, params)
  }
   ...
   * }
   */
  function createNetwork ({ name, type, timeZone, tags }) {
    const data = { name, type, timeZone, tags }
    return axios._post(apiKey, target, `${basePath}/organizations/${organizationId}/networks`, data)
  }
